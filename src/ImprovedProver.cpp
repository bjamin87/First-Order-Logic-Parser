...
ImprovedConfig loadImprovedConfig(const std::string& filename) {
...
RuleConfig getRuleConfig(const ImprovedConfig& cfg, const std::string& ruleName) {
...
bool isManualRule(const ImprovedConfig& cfg, const std::string& ruleName) {
    return getRuleConfig(cfg, ruleName).mode == "manual";
}

bool isHeuristicRule(const ImprovedConfig& cfg, const std::string& ruleName) {
    return getRuleConfig(cfg, ruleName).mode == "heuristic";
}

bool isDisabledRule(const ImprovedConfig& cfg, const std::string& ruleName) {
...
std::string freshConstantImproved() {
    static int counter = 0;
    ++counter;
    return "ic" + std::to_string(counter);
}

std::string makeInstantiationKeyImproved(
...
int scoreRuleOption(const RuleOption& option, const ImprovedConfig& cfg) {
    int score = 0;
        ...
        for (std::size_t i = 0; i < sequent.getRight().size(); ++i) {
            ...
            if (formula && formula->getType() == FormulaType::Forall) {
                ...
                if (var != nullptr && body != nullptr) {
                    std::string c = freshConstantImproved();
                ...
                if (formula != nullptr && formula->getType() == FormulaType::Forall) {
                    ...
                    if (var != nullptr && body != nullptr) {
                        for (const std::string& term : terms) {
                            std::string key = makeInstantiationKeyImproved(formula, term, "forallL");
        ...
        if (!isDisabledRule(cfg, "existsR_used")) {
            for (std::size_t i = 0; i < sequent.getRight().size(); ++i) {
                ...
                if (formula != nullptr && formula->getType() == FormulaType::Exists) {
                    ...
                    if (var != nullptr && body != nullptr) {
                        for (const std::string& term : terms) {
                            std::string key = makeInstantiationKeyImproved(formula, term, "existsR");
                            if (usedInstantiations.find(key) != usedInstantiations.end()) {
                                continue;
                            }
    ...
    if (!isDisabledRule(cfg, "forallL_fresh")) {
        for (std::size_t i = 0; i < sequent.getLeft().size(); ++i) {
            ...
            if (formula != nullptr && formula->getType() == FormulaType::Forall) {
                ...
                if (var != nullptr && body != nullptr) {
                    std::string freshKey = makeInstantiationKeyImproved(formula, "__fresh__", "forallL");
    ...
    if (!isDisabledRule(cfg, "existsR_fresh")) {
        for (std::size_t i = 0; i < sequent.getRight().size(); ++i) {
            ...
            if (formula != nullptr && formula->getType() == FormulaType::Exists) {
                ...
                if (var != nullptr && body != nullptr) {
                    std::string freshKey = makeInstantiationKeyImproved(formula, "__fresh__", "existsR");
                    if (usedInstantiations.find(freshKey) != usedInstantiations.end()) {
                        continue;
                    }
...
std::string ImprovedProver::getName() const {
    return "ImprovedProver";
}

ProofResult ImprovedProver::prove(ProofState& state) {
    auto start = std::chrono::high_resolution_clock::now();

    ImprovedConfig cfg = loadImprovedConfig("heuristics.txt");
    ...
    bool proved = proveSequentImproved(
        sequent,
        steps,
        branches,
        usedInstantiations,
    ...
    std::string message = proved
        ? "Improved (" + cfg.profileName + "): hybrid search succeeded"
        : "Improved (" + cfg.profileName + "): hybrid search failed";
